...
from yolov5.utils.datasets import LoadImages, LoadStreams, VID_FORMATS, IMG_FORMATS
...
def detect(opt):
    ...
    with js3:
        st.markdown("**显存使用率**")
        js3_text = st.markdown("0MB")

    model.warmup(imgsz=(1 if pt else nr_sources, 3, *imgsz))  # warmup
    dt, seen = [0.0, 0.0, 0.0, 0.0], 0
    prev_time = time.time()
    for frame_idx, (path, im, im0s, vid_cap, s, pro) in enumerate(dataset):
        t1 = time_sync()
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if webcam:  # nr_sources >= 1
                ...
                if source.endswith(VID_FORMATS):
                    ...
                    save_path = str(save_dir / p.name)  # im.jpg, vid.mp4, ...
                # image file
                if source.endswith(IMG_FORMATS):
                    txt_file_name = p.stem
        ...
        kpi2_text.write(str(int(n)))
        kpi3_text.write(str(max_id))
...
def parse_opt_d():
    ...
    parser.add_argument('--fourcc', type=str, default='avc1', help='output video codec (verify ffmpeg support)')
